...
使用命令 `npx create-react-app steps` 创建项目。
...
- 组件在一段时间内可以保存的数据，这些数据对于在应用的整个生命周期中需要记住的信息来说是必要的。
- 组件状态:单个局部组件变量。
- 更新组件状态触发器React以重新呈现组件。

### 状态机制(Mechanics)

我们不直接操作DOM，我们通过 `useState` 更新状态，然后react重新渲染组件。

这也是被称为`React`的原因，通过重新渲染用户界面来对状态变更做出反应。

### 状态实用指南(Practical Guideline)

每个组件都有并且管理自己的状态，无论渲染同一个组件多少次。

- 对于组件应该随时间跟踪(“记住”)的任何数据，使用状态变量。这是在某个时间点会改变的数据。在原生JS中，这是一个let变量，或者是 [] 或者是 {}。
- 每当您希望组件中的某个东西是动态的，就创建一段与该“东西”相关的状态，并在该东西”应该改变时更新状态(也就是“动态的”)。
- 如果要更改组件的外观或显示的数据，请更新其状态。这通常发生在事件处理函数中。
- 构建组件时，请将其视图想象为随时间变化的状态的反映。
- 对于不应该触发组件重新呈现的数据，不要使用状态。用常规变量代替。这是初学者常犯的错。
...
- Props
  ...
  - 类似于函数参数。
  - 只读。
  - 接收新Props会导致组件重新渲染。通常当父级的状态已被更新时。
  - 由父级用于配置子组件。
